...
现已增加Ctrl-Alt-Shift-K快捷键退出功能，为确保安全性，需连按5次，且需输入密码才能退出（首次使用需设置密码，重置密码需要使用其他方法退出后再使用--reset参数重置，需要验证Windows登录密码才能进行下一步重置操作）
...
静音只会对物理音频设备进行静音，但软件仍可捕获播放中的音频流，故静音不会影响到远程串流时的音频播放，适用于在远程连接期间不想让其他人在实体机端听到声音的用户。
